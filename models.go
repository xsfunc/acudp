...
// SessionType represents the type of the session (Booking, Practice, Qualifying, Race).
...
// Event represents the type of ACUDP event.
type Event uint8
...
// Message is the interface implemented by all UDP messages.
...
func NewAdminCommand(data string) (*AdminCommand, error) {
	// Note: Admin commands might need full charset, but keeping consistent with others for now
	// If full charset is needed, remove sanitizeString here.
	data = sanitizeString(data)
	encoded, err := encodeUTF32(data)
	if err != nil {
		return nil, err
